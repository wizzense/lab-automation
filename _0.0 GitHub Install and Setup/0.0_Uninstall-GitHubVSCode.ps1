...
function Uninstall-GitHubCLI {
    ...
    if (Test-Path $ghPath) {
        ...
        # Uninstalling the MSI package using its product code or MSI file
        Start-Process -FilePath "msiexec.exe" -ArgumentList "/x $ghPath /qn" -Wait
        Write-Host "GitHub CLI uninstallation completed."
    } else {
